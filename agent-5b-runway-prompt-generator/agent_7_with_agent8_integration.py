"""
Agent 7 (Runway Gen-4 Prompt Optimizer) with Agent 8 (Prompt Validator) Integration

This module integrates Runway Gen-4 prompt generation with Agent 8's validation pipeline.
Ensures all Runway prompts are validated before video generation.

Flow:
1. Generate Runway Gen-4 prompt with motion/style parameters
...
class Agent7RunwayWithAgent8Integration:
    """Agent 7 (Runway Optimizer) with Agent 8 (Validator) Integration"""

    def __init__(self, agent8_url: str = "http://localhost:5000/validate"):
        """
        Initialize Agent 7 with Agent 8 integration
        ...
        logger.info(f"🔗 Agent 7↔8 Integration initialized (Agent 8: {agent8_url})")

    def generate_runway_prompt(self, scene_description: str, style_anchors: Dict, genre: str) -> Dict:
        """
        Agent 7 Standard: Generate Runway Gen-4 Prompt

        Args:
            scene_description: Description of the scene to generate
            style_anchors: Visual style parameters (motion, camera, etc.)
            genre: Music genre (pop, rock, hip-hop, etc.)

        Returns:
            Dict containing Runway Gen-4 prompt structure
        """
        logger.info(f"📝 Agent 7: Generating Runway Gen-4 Prompt for {genre}")

        # Extract style parameters with defaults
        motion_intensity = style_anchors.get("motion_intensity", 0.5)
        camera_movement = style_anchors.get("camera_movement", "smooth")
        shot_type = style_anchors.get("shot_type", "medium")
        visual_style = style_anchors.get("visual_style", "cinematic")

        # Build Runway Gen-4 optimized prompt
        runway_prompt = {
            "type": "runway_gen4",
            "prompt": f"Runway Gen-4 | {genre.upper()} Music Video: {scene_description}. "
                     f"{visual_style.capitalize()} {shot_type} shot, "
                     f"{camera_movement} camera movement, "
                     f"motion intensity {motion_intensity}. "
                     f"High-quality video output, smooth transitions, professional grade.",
            "parameters": {
        ...
        logger.info(f"✓ Runway Prompt generated: {len(runway_prompt['prompt'])} chars")
    ...
    def validate_with_agent8(self, prompt: Dict) -> Optional[Dict]:
        ...
        Args:
            prompt: Runway prompt dictionary to validate

        Returns:
            Agent 8 validation result or None if failed
        """
        logger.info(f"🔄 Agent 7→8: Sending prompt to Agent 8 for validation")
    ...
    def generate_and_validate_runway_prompt(
        self,
        ...
        This is the primary method to use for creating validated Runway prompts.
        ...
        """
        logger.info(f"🚀 Starting Runway Prompt Generation & Validation Pipeline")

        # Step 1: Generate
        prompt = self.generate_runway_prompt(scene_description, style_anchors, genre)
...
if __name__ == "__main__":
    ...
    integrator = Agent7RunwayWithAgent8Integration(
        agent8_url="http://localhost:5000/validate"
    )

    # Example prompt generation
    scene = "Close-up of artist singing emotionally, dramatic lighting, slow motion"
    style = {
    ...
    result = integrator.generate_and_validate_runway_prompt(
